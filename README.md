...
Docs are generated automatically from component source files and Storybook stories — no Storybook build required.
...
- Component attributes and types → [Custom Elements Manifest](package/custom-elements.json) (generated by `pnpm analyze`)
- Slots → JSDoc `@slot` tags in each `src/components/*/index.ts`
- Usage examples → extracted statically from each component's `*.stories.ts` file
- Typography classes → `src/shared/styles/typography-v4.css`
...
```sh
pnpm analyze   # regenerate custom-elements.json first (if not already done)
pnpm docs:llms
```
