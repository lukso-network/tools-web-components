...
export class LuksoCheckbox extends TailwindElement {
    ...
    checkedIconTemplate(): unknown {
        return html`
            <lukso-icon
                    ...
                    name="tick"
                    color="neutral-20"></lukso-icon>
        `
    }

    checkboxTemplate(): unknown {
        return html`
            <div class=${customClassMap({
    ...
    render() {
        return html`
            <div
            >
                <label for=${this.name} class=${this.defaultContainerStyles} @mouseenter=${this.handleMouseOver}
