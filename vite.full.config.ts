...
/**
 * Write auto-generated CSS files for colors and fonts
 *
 * Generates:
 * - colors.scss: For Sass/v3 builds (@theme directive with color variables)
 * - colors-v4.css: For Tailwind v4 builds (imported by main-v4.css)
 * - fonts-v4.css: Font faces for Tailwind v4 builds (imported by main-v4.css)
 *
 * Colors are generated from JavaScript config to stay in sync.
 * Fonts use configurable paths (default: /assets/fonts).
 */
async function writeSass() {
  // Write colors.scss for Sass builds (v3 compatibility)
  const scssPath = path.join(__dirname, 'src/shared/styles/colors.scss')
  const oldScss = await readFile(scssPath, 'utf-8')
  ...
  const newScss = scssOutput.join('\n')

  if (oldScss !== newScss) {
    console.log(`writing ${scssPath}`)
    await writeFile(scssPath, newScss)
  }

  // Write colors-v4.css for Tailwind v4 builds
  const cssPath = path.join(__dirname, 'src/shared/styles/colors-v4.css')
  const oldCss = await readFile(cssPath, 'utf-8').catch(() => '')
  ...
  for (const [group, colorValues] of Object.entries(colorPalette)) {
    for (const [color, value] of Object.entries(colorValues)) {
      cssOutput.push(`  --${group}-${color}: ${value};`)
    }
  }
  cssOutput.push('}', '')

  const newCss = cssOutput.join('\n')

  if (oldCss !== newCss) {
    console.log(`writing ${cssPath}`)
    await writeFile(cssPath, newCss)
  }

  // Write fonts-v4.css for Tailwind v4 builds
  const fontsPath = path.join(__dirname, 'src/shared/styles/fonts-v4.css')
  const oldFonts = await readFile(fontsPath, 'utf-8').catch(() => '')

  const fontsCss = `/* DO NOT MODIFY MANUALLY. This file is auto generated by Vite build process. */
@font-face {
  font-family: Inter;
...
`
  ...
  if (oldFonts !== fontsCss) {
    console.log(`writing ${fontsPath}`)
    await writeFile(fontsPath, fontsCss)
  }
}
